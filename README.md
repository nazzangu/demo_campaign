...
### 캠페인 빌더
드래그 앤 드롭으로 캠페인 플로우를 설계하는 메인 화면입니다.
![캠페인 빌더](screen.jpg)

### 캠페인 운영 현황
운영 중인 캠페인의 진입/여정 중/이탈/완료 통계를 실시간으로 확인합니다.
![캠페인 운영 현황](campaign_progress.jpg)

### 푸시 메시지 설정
앱 푸시 메시지 작성, 휴대폰 미리보기, 발송 조건/주기 설정 화면입니다.
![푸시 메시지 설정](push_setting.jpg)

### 알림톡 메시지 설정
카카오톡 채널 선택, 메시지 내용 작성, 발송 제한 시간 및 수신 동의 설정 화면입니다.
![알림톡 메시지 설정](kko_setting.jpg)

### 알림톡 템플릿 선택
카카오톡 알림톡 템플릿 검색, 상세 내용 확인 및 미리보기 모달입니다.
![알림톡 템플릿 선택](kko_template.jpg)

### 문자메시지 설정
SMS/LMS/MMS 유형 선택, 메시지 작성, 미리보기, 발송 제한 및 수신 동의 설정 화면입니다.
![문자메시지 설정](sms_setting.jpg)

### 사용자 기반 분기
사용자 속성(gender, membership, 지역 등) 조건으로 그룹을 분리하는 분기 설정 화면입니다.
![사용자 기반 분기](user_segment.jpg)

### 이벤트 기반 분기
특정 이벤트 발생 여부와 속성 필터로 사용자 경로를 분기하는 설정 화면입니다.
![이벤트 기반 분기](event_segment.jpg)

### 세그먼트 (코호트) 설정
코호트 규칙 빌더로 이벤트/기간/집계/속성 필터를 AND/OR 조합하여 대상 사용자를 정의합니다.
![세그먼트 설정](segment.jpg)
...
### 설정 관리
- 6개 탭: 이벤트, 속성, 푸시 앱, 개인화 변수, 채널, 메시지 템플릿
- 항목별 CRUD (추가/수정/삭제)
...
| 경로 | 페이지 | 설명 |
|------|--------|------|
| `/` | 캠페인 목록 | 캠페인 카드 목록, 생성, 운영 현황/설정 이동 |
| `/campaigns/:id/builder` | 캠페인 빌더 | 플로우 에디터, 노드 설정, 상태별 액션 |
| `/ops` | 운영 현황 | 상태 요약, 시작/일시정지/중지/폐기 관리 |
| `/settings` | 설정 관리 | 이벤트/속성/푸시앱/변수/채널/템플릿 CRUD |
